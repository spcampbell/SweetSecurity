...
def install(chosenInterface,webServer):
	...
	#https://www.zeek.org/downloads/bro-2.6.2.tar.gz
	broLatest='2.6.2'
	...
	if broInstalled==False:
		...
		print "  Downloading Bro IDS 2.6.2"
		os.popen('sudo wget https://www.zeek.org/downloads/bro-2.6.2.tar.gz 2>&1').read()
		if not os.path.isfile('bro-2.6.2.tar.gz'):
			sys.exit('Error downloading Bro')
		if not hashCheck.checkHash('bro-2.6.2.tar.gz'):
			sys.exit('Error downloading Bro, mismatched file hashes')
		print "  Unpacking Bro Code"
		os.popen('sudo tar -xzf bro-2.6.2.tar.gz').read()
		...
		os.chdir(cwd)
		os.remove('bro-2.6.2.tar.gz')
		shutil.rmtree("bro-2.6.2/")
